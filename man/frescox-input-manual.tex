...
%
\title{\Huge Fresco}
\date{Version FRESCOX.7.3, July 2025s\\ LLNL-CODE-811517 }
\author{I.J. Thompson
%
...
\hangindent 8ex\hangafter 1  MELFIL
...
\\
 for\>TYPE\\
\>0\> \>Coulomb potential (see Card 9 above)\\
...
\\  IT is `1' or `3' to make this component included only iteratively,\\
 ...
 couplings,\\
 \hspace*{8mm} `-' or `3' to NOT use this form to couple channels (only  to make other forms, e.g. deformations).
...
The order of form factors in the file must agree with the order called by a double loop of channels, with the `to' channel number varying more rapidly.
\\
\hangindent 8ex\hangafter 1 IP4 
