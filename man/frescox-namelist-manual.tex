...
\>18\>     \>Generate deformed spin-orbit forces from preceding TYPE=11 target deformations.\\
...
IT is 1 or 3 to make this component included only iteratively,\\
 ...
 \hspace*{8mm} `-'  to NOT use this form to couple channels (only  to make other forms, e.g. deformations).

\noindent
NOUSE true (`T') to NOT USE this form to couple channels (only  to make other forms, e.g. deformations),
...
For TYPE=18  generate deformed spin-orbit couplings from preceding TYPE=11 target 
deformations.  The TYPE=11 namelist and the used central potential themselves should
have nouse=T to remove those potentials  themselves  being included.
Rescale the real part by P1, the imaginary part by P2.
...
\newpage
\item[KIND]
The couplings can be of 12 KINDs:

            = 0 : specific potential shape for specific transition\\
...
\paragraph{For KIND = 0} 
\underline{General Potential Shape}\\
For local external form factors on FILE `INFILE' [default 4]), to be given 
coupling factors from the same {\tt TENSOR} routine as used for \&POT components
described in section \ref{potentials}.\\
...
read FROM FILE `INFILE'  the following lines
   (until a blank line):
\begin{verbatim}
Format 16.2: I4, 3F8.4,          5I4,               3I4,     4A8
             NP, HNP,RFS,FSCALE, TYPE,k,TAU,DER,IT, IB,IA,M, COMMENT
 for NP = number of radial points
    HNP = step size
    RFS = radius of first point
    FSCALE = scaling factor to be applied (effective if M=0)
    TYPE = type of potential in &POT section
    k = multipole 1..6, with monopoles 0 diagonal and 7 off-diagonal
    TAU =  part of Thomas deformed projectile spin-orbit in TYPE=18
                1 for normal potential derivative,  dV/dr / r
                2 for wf derivative, V/r d/dr
                3 for V/(2r*r) 
    DER = 0 for radial scalar, and 1 or TAU=2 for radial derivative.
    IT = 1 or 3: make this component included only iteratively
         2 or 3: NOT subtract this in KIND=3,4 single-particle couplings
    IB  = excited-state pair fed by coupling 
    IA  = excited-state pair feeding the coupling
    M = index to previous form read in with KIND=0 (M=0 if none).
            (Note: if M>0, then current FSCALE is taken as 1.0)
    COMMENT = comment for display in printout.
\end{verbatim}
%If LOP, DER cannot be read without error, they will be assumed both to be 0.\\
% Then free format for the radial shapes, repeatedly until sufficient data is read.\\
 
 If IP1 = 0 and M = 0, read local form factor from i=1 to NP.
\\  1, read successively for j=1,NLO the non-local form factors  FNL(i,j), i=1 to NP   [NLO = RNL / max(HNL,HCM)]
\\

List of supported potential TYPEs:
\begin{verbatim}
   0  Central Coulomb
   1  Central Volume
   2  Central Derivative
   3  Projectile Spin-orbit
   4  Target spin-orbit
   5  Projectile Tr tensor
   6  Target Tr tensor
   7  Joint spin-spin tensor
   8  Joint spin-spin scalar
   9  1 - Effective-mass
  10  Projectile Deformed
  11  Target Deformed
  18  Thomas projectile spin-orbit 
 \end{verbatim}
%  12  Projectile table
%  13  Target table
%  14  Projectile second-order table
%  15  Target second-order table
%  16  Mutual second-order
%  17  Projectile-target all-order matrix
%  24  Central Volume  for (N-Z)/A Lane diagonal
%  25  Central Derivative  for (N-Z)/A Lane diagonal
%  26  Central Volume  for (N-Z)/A Lane coupling
%  27  Central Derivative  for (N-Z)/A Lane coupling
%  30  Central L(L+1)
...
Format 16.2: I4, 3F8.4,          I4,  2F4.0,   4I4,            4A8
             NP ,HNP,RFS,FSCALE, LTR, PTR,TTR, IB,IA, LOP,DER, COMMENT
 for NP = number of radial points
    HNP = step size
    RFS = radius of first point
    ...
    DER = 0 for radial scalar interaction, 1 for radial derivative
    COMMENT = comment for display in printout.
\end{verbatim}
If LOP, DER cannot be read without error, they will be assumed both to be 0.\\
 Then free format for the radial shapes, repeatedly until sufficient data is read.\\
 ...
 If IP1 =
      0, read local form factor from i=1 to NP
